...
def _apply_alias_map(strings, aliases, inplace=False):
    ...
    alias_map = pd.Series(
        alias_map['node_id_y'].array,
        index=alias_map['node_id_x'].array
    )

    string_aliased = strings['node_id'].isin(alias_map.index)
    mapped_nodes = strings.loc[string_aliased, 'node_id']

    if inplace:
        strings.loc[string_aliased, 'node_id'] = alias_map[mapped_nodes].array
    else:
        output = strings.copy()
        output.loc[string_aliased, 'node_id'] = alias_map[mapped_nodes].array
        return output
...
def map_aliases(strings, aliases, case_sensitive=False):
    ...
    if 'node_type' in strings.columns:
        if len(strings['node_type'].value_counts()) > 1:
            raise ValueError(
                'Input strings frame contains references to more than '
                'one node type. Aliases must be mapped for a single '
                'node type.'
            )
        ...
        raise ValueError(
            'input strings frame must contain a column labeled '
            'node_type or node_type_id.'
        )

    if aliases['key'].isin(aliases['value']).any():
        raise ValueError(
            'Aliases has strings that appear as both keys and values.'
        )
    ...
    key_is_new = (
        ~aliases['key'].isin(strings['string'])
    ...
    if aliases['key'].duplicated().any():
        raise ValueError(
            'Aliases contains key(s) that map to multiple values when '
            'case_sensitive={}. Aliases cannot be a one-to-many or '
            'many-to-many map.'.format(case_sensitive)
        )
    ...
    if not key_is_new.empty:
        ...
        aliases_w_new_keys.rename(
            columns={'index': 'alias_index'},
            inplace=True
        )
        string_map = aliases_w_new_keys.merge(
            strings,
            left_on='value',
            right_on='string'
        )
        ...
        new_strings_frame = pd.DataFrame({
            'node_id': string_map['node_id'].array,
            'string': new_strings
        })
        new_strings_frame.drop_duplicates(inplace=True)

        new_string_ids = bg.non_intersecting_sequence(
            new_strings_frame.index,
            strings['string_id']
        )
...
def _make_new_nodes_table(tn, node_data):
    ...
    if nodes['node_id'].duplicated().any():
        raise ValueError(
            'a node_id has been assigned more than one node_type_id'
        )
    ...
    def group_max_min_length(group):
        ...
        node_id = group.name
        return pd.Series(
            [node_id, max_string_id, min_string_id],
            index=columns
        )
...
def _make_new_assertions_table(tn,
    ...
    sources = node_data.loc[
        ~string_is_assrtn_tgt,
        [assrtn_tgt_label, 'string_id', 'link_type']
    ]
    assertions = sources.merge(
        targets,
    ...
    assertions.rename(
        columns={'string_id_src': 'src_string_id',
...
def _normalize_metadata(
    ...
    return {
        node_type: get_node_metadata(node_type)
...
def _make_new_node_md_table(tn, node_type, md_table):
    ...
    node_type_id = tn.get_element_by_value(
        'node_type',
        node_type,
        'node_type_id'
    )
    ...
    if not md_table.empty:
        ...
        md_table = tn.nodes.loc[
            tn.nodes['node_type_id'] == node_type_id,
            ['node_id', 'node_type_id']
        ]
...
def _make_md_strings_and_assrtns(
    tn,
    node_md_table,
    node_data,
    assrtn_tgt_label
    ...
    md_link_id = tn.get_element_by_value(
        'link_type',
        'metadata',
        'link_type_id'
    )

    new_assrtns = pd.DataFrame(
        {'assertion_id': assrtn_ids,
...
def create_textnet_from_node_data(
    node_data,
    assrtn_tgt_label,
    ...
    assertions = _make_new_assertions_table(
        tn,
        node_data,
        assrtn_tgt_label
    )
    ...
    # make edges from non-title assertions and insert them into
    # network
    title_link_type_id = tn.get_element_by_value(
        'link_type',
        'title',
        'link_type_id'
    )
    ...
    if metadata is not None:
        ...
        contains_separator = metadata.astype(
            dict(zip(metadata.columns, [str]*len(metadata.columns)))
        )

        def check_sep(x):
            return x.str.contains(tn.md_sep)

        contains_separator = contains_separator.apply(check_sep)
        if contains_separator.any().any():
            raise ValueError(
                'Metadata values contain the string "{}", which is '
                'the current separator for values in metadata '
                'strings. Use the md_sep keyword or change the source '
                'files to eliminate the conflict.'.format(tn.md_sep)
            )
        ...
        for node_type, md_table in metadata.items():
            ...
            node_md_table = _make_new_node_md_table(
                tn,
                node_type,
                md_table
            )
            ...
            columns_in_order = [
                c for c in tn.schema[node_type].columns
            ...
            md_str_assrt = _make_md_strings_and_assrtns(
                tn,
                node_md_table,
                node_data,
                assrtn_tgt_label
