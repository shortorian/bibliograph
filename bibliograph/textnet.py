...
class TextNet:
    ...
    def __init__(self,
        ...
        if node_types_fname is None:
            node_schema = self._make_node_type_tables(bg._default_node_types_fname)
        else:
        ...
        if not (has_link_type ^ has_md_column).all():
            raise ValueError('for every row of the label map, there must be a '
                             'value in column "link_type" or a value in '
                             'column "metadata_column", but not both.')
        ...
        elif bg._default_tags_fname != False:
        ...
        self._map_to_table_names = pd.Series({'source':'sources',
                                              'source_id':'sources',
        ...
        for n_type in self.node_types['node_type']:
            try:
                ...
                raise RuntimeError('TextNet object failed integrity check. '
                                   '"{}" is listed in node_types["node_type"] '
                                   'but it is not an attribute of this '
                                   'TextNet object.'.format(n_type))
    ...
    def _make_node_type_tables(self, filename):
        ...
        if len(table_data) == 0:
            raise ValueError('Found an object of length zero in file {}'
                            .format(filename))
        ...
        node_types = pd.DataFrame({'node_type_id':node_type_ids,
                                   'comment_id':node_type_ids,
                                   'node_type':table_names,
                                   'comment':type_comments},
                                  index=node_type_ids)
                                  
        node_types, comments = bg.extract_comments(node_types,
                                                   extract_from_schema='node_types',
                                                   template_tn=self,
                                                   insert_columns=True)

        table_data = {key:{k:v for k,v in val.items() if k != 'comment'}
        ...
        for name,cols in table_data.items():
            ...
            if not_list_like or not_two_valued:
                raise ValueError('Node type table specifications must be of the '
                                'form '
                                'name:{{column:[SQL_DTYPE, pandas_dtype]}}. '
                                'Found invalid column specification {}:{} in '
                                'file {}.'.format(name, cols, filename))

        pandas_dtypes = {name:{k:v[1] for k,v in cols.items()}
                         for name,cols in table_data.items()}
        
        nodes_dtypes = dict(zip(self.nodes.columns, self.nodes.dtypes))
        for k,v in pandas_dtypes.items():
            these_types = {'node_id':nodes_dtypes['node_id'],
                           'node_type_id':nodes_dtypes['node_type_id']}
            these_types.update(v)
            pandas_dtypes[k] = these_types

        tables = {name:pd.DataFrame(columns=dtypes.keys()).astype(dtypes)
    ...
    def _read_link_types(self, filename):
        ...
        if len(link_types) == 1:
            link_types = pd.DataFrame({'comment':comments,
        ...
        link_types, comments = bg.extract_comments(link_types,
        ...
        if not comments.empty:
            ...
            normalized_id = bg.non_intersecting_sequence(naive_id,
        ...
        normalized_id = bg.non_intersecting_sequence(len(link_types['link_type_id']),
    ...
    def _read_node_tags(self, filename):
        ...
        tags, comments = bg.extract_comments(tags,
    ...
    def append_to(self, table_name, data, comment=None, **kwargs):
        ...
        if type(data) == pd.Series:
            ...
            ignore_index_false = (kwargs['ignore_index']==False)
            if missing_ignore_index_kw or ignore_index_false:
                raise ValueError('if passing a pandas Series, '
                                 'you must set ignore_index=True')
        ...
        if comment is not None:
            ...
            if not existing.columns.str.contains('comment').any():
                raise ValueError('table {} has no comment_id column. cannot '
                                 'process comment {}'
                                 .format(table_name, comment))
            ...
            try:
                ...
                if 'does not exist' in str(e):
                    ...
                    new_comment_row = pd.Series({'comment_id':comment_id,
                                                'comment':comment,
                                                'date_inserted':pd.NA})
    ...
    def get_table_by_label(self, label, search=False):
        try:
            ...
            if label in self.schema.keys():
                ...
                has_column_label = [name for name,table in self.schema.items() 
                                    if (label in table.columns)]

                if len(has_column_label) > 1:
                    raise ValueError('Column label {} found in more '
                                     'than one table.'.format(label))

                elif len(has_column_label) == 1:
                    table_name = has_column_label[0]

                else:
                    raise ValueError('TextNet object has no tables '
                                     'with column label {}.'.format(label))

            else:
                raise KeyError('TextNet object has no table named {} '
                               'and no key {} in_map_to_table_names.'
                               .format(label))
    ...
    def get_row_by_value(self, unique_column, unique_value):
        ...
        if len(match) > 1:
            raise ValueError('Value "{}" is not unique in {}["{}"]'
                             .format(unique_value, table.name, unique_column))
        elif len(match) == 1:
            return match.squeeze()
        else:
            raise ValueError('Value "{}" does not exist in {}["{}"]'
                             .format(unique_value, table.name, unique_column))

    
    def get_element_by_value(self,
                             unique_column,
                             unique_value,
                             desired_column):
    ...
    def get_value_by_name(self,
        ...
        if name_label not in table.columns:
            raise ValueError('cannot get {0} ID by name because table '
                             '{0} has no column labeled "{1}".'
                             .format(table_name, name_label))
        ...
        if len(matched_rows) > 1:
            raise ValueError('Name "{}" is not unique in {}["{}"]'
            ...
            if column_name is not None:
                if row is not None:
                    raise ValueError('Provide a column name or set row=True '
                                     'or neither, not both.')
            ...
            raise NameError('row name "{}" does not exist in {}["{}"]'
                            .format(row_name, table_name, name_label))
    ...
    def merge_strings_tables(self, new, update=False):
        ...
        if len(common_cols) == 0:
            raise ValueError('new strings frame must have columns in '
                             'common with existing strings frame')

        new.loc[:, 'date_inserted'] = bg.time_string()

        self.append_to('strings',
                       new.loc[~new['string'].isin(self.strings['string']), :])


    def map_aliases(self, aliases, case_sensitive=False):

        has_type = self.strings.merge(self.nodes[['node_id', 'node_type_id']],
                                      on='node_id')
    ...
    def merge_and_replace(self,
        ...
        if other_is_left:
            output = other.merge(table[[right_on, replace_with]],
                                 left_on=left_on,
                                 right_on=right_on,
                                 **kwargs)
        else:
            output = table.merge(other[[right_on, replace_with]],
                                 left_on=left_on,
                                 right_on=right_on,
                                 **kwargs)
    ...
    def resolve_nodes(self, string_type=None, selection=None, drop=False):
        ...
        if string_type == 'name':
            ...
            raise ValueError('Unrecognized node string type {}. '
                             'Must be "name", "abbreviation", or "all".'
                             .format(string_type))
        ...
        cols_dict = {'date_inserted':'string_date_inserted',
        ...
        if string_type == 'all':
            ...
            strings = strings[[c for c in strings.columns
                               if c !='node_id']]
            output = output.merge(strings,
    ...
    def resolve_edges(self, string_type=None, selection=None, drop=False):
        ...
        node_is_relevant = pd.concat([output['src_node_id'],
                                      output['tgt_node_id']])
        node_is_relevant.drop_duplicates()
        node_is_relevant = self.nodes['node_id'].isin(node_is_relevant)

        nodes = self.resolve_nodes(string_type=string_type,
                                   selection=node_is_relevant,
                                   drop=drop)
        nodes.rename(columns={'comment_id':'node_comment_id',
    ...
    def resolve_assertions(self, selection=None, drop=False):
        ...
        output = output.merge(self.link_types[['link_type_id', 'link_type']])
        is_src = self.strings['string_id'].isin(self.assertions['src_string_id'])
        src_strings_subset = self.strings.loc[is_src]
        is_tgt = self.strings['string_id'].isin(self.assertions['tgt_string_id'])
    ...
    def resolve_node_type(self,
        ...
        if node_type not in self.node_types['node_type'].array:
            raise ValueError('{} is not listed in node_types["node_type"]'
                             .format(node_type))

        table = self.__getattribute__(node_type)
        table = table[[l for l in table.columns if l !='node_type_id']]
...
def _check_schema_cols(schema, schema_script, table_name):
    if table_name in schema.keys():
        ...
        if not all([l in default_cols for l in user_cols]):
            raise ValueError('Table {0} defined in {1} is missing required '
                             'columns. The {0} table must at least have '
                             'columns {2}.'
                             .format(table_name, schema_script, default_cols))
